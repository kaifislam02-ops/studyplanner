...
export default function Sidebar({ 
  ...
  return (
    <aside className={`w-[320px] ${bgClass} border-r ${borderClass} flex flex-col h-screen overflow-hidden`}>
      {/* Logo - Fixed */}
      <div className={`p-6 border-b ${borderClass} flex-shrink-0`}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className={`px-4 py-3 border-t ${borderClass} space-y-2`}>
          <button
            ...
            className={`w-full ${cardBg} hover:bg-white/10 rounded-lg px-3 py-2 text-sm font-medium text-left flex items-center gap-2 transition-colors`}
          >
            ⏱️ Pomodoro Timer
          </button>
          <button
        ...
        {/* Subjects */}
        <div className={`px-4 py-3 border-t ${borderClass} pb-6`}>
          ...
          {/* Subjects List - Scrollable within subjects section */}
          <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            {subjects.map(subject => (
              <div
                key={subject.id}
                className={`flex items-center gap-2 p-2 rounded-lg hover:${cardBg} group transition-colors`}
              >
                <div
            ...
            <div className={`mt-3 p-3 ${cardBg} rounded-lg space-y-2`}>
              ...
              <div className="flex gap-1 flex-wrap">
                {PRESET_COLORS.map(color => (
                  <button
                    ...
                    className={`w-6 h-6 rounded transition-all ${newSubject.color === color ? 'ring-2 ring-blue-500 scale-110' : 'hover:scale-105'}`}
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 bg-blue-500 hover:bg-blue-600 rounded px-3 py-1.5 text-xs font-medium text-white transition-colors"
                >
                  Add
                </button>
                <button
                  onClick={() => setShowAddSubject(false)}
                  className={`px-3 py-1.5 text-xs ${textMuted} hover:${darkMode ? 'text-white' : 'text-gray-900'} transition-colors`}
                >
                  Cancel
                </button>
              </div>
